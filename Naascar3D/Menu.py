...
class Menu:
    def __init__(self):
        ...
        pygame.display.init()
        pygame.display.set_mode((self.settings["aspect_x"], self.settings["aspect_y"]), pygame.DOUBLEBUF|pygame.OPENGL)
    ...
    def click_button(self, action):
        if action == "start_menu":
            ...
            GameManager(self.settings).start()
    ...
    def display(self, buttons):
        ...
        GL.glLoadIdentity()
        GL.glViewport(0, 0, self.settings["aspect_x"], self.settings["aspect_y"])
        GLU.gluOrtho2D(self.settings["viewport"]["x"], self.settings["viewport"]["width"], self.settings["viewport"]["y"], self.settings["viewport"]["height"])

        #Logo
        self.display_logo()
        self.text.display_text(450, 1200, "NAASCAR 3D", 20)
        #Buttons
        for button, info in buttons.items():
            x, y = info["pos"]
    ...
    def display_back_arrow(self):
        ...
        for c in range(len(self.back_arrow_image)):
            GL.glVertex2f(self.back_arrow_image[c][0] + posx - 25, self.back_arrow_image[c][1] + posy - 20)
        GL.glEnd()
    ...
    def draw_corner(self, center_x, center_y, radius, start_angle, end_angle, segments):
        ...
        GL.glBegin(GL.GL_TRIANGLE_FAN)
        GL.glVertex2f(center_x, center_y)  # Center point
